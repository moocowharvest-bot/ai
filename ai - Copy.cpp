...
std::string pickRandomColorNoBurgundy()
{
	std::string color = pickRandomString({
	   "darkblue", "darkpurple", "white", "pink", "dark_gray", "black", "dark_green"
	});
	color += " ";
...
std::string pickRandomColorNoDarkGreen()
{
	std::string color = pickRandomString({
...
std::string pickUpper(std::string output)
{
	...
	if (output.find("back") != std::string::npos) {
		upper += pickRandomString({
			...
			"button down shirt, " + pickRandomColor() + pickRandomMaterial() + "skin-tight skirt",
		});
...
std::string pickLower(std::string output)
{
	...
	if (output.find("dress") != std::string::npos || output.find("robe") != std::string::npos) {
		...
		lower += pickRandomString({
			...
			"panty briefs",
			"thong",
			"unzipped " + pickRandomColor() + "jeans exposing " + pickRandomColor() + pickRandomMaterial() + "panties",
			"unzipped " + pickRandomColor() + "pants exposing " + pickRandomColor() + pickRandomMaterial() + "panties",
			"yoga pants in a gym with a water bottle",
			"jeans",
			"cheeky panties",
			"socks, naked",
			"sleepshorts",
...
//		newPose.push_back("((one " + woman + ")), (((pov of a man groping)))");
	...
	newPose.push_back("((one " + woman + ")), (((floating ethereal ghost hand)))");
	
	newPose.push_back("((one " + woman + ")), (((a man is massaging the sleeping woman's " + fondleTarget + ":1.5)))");
...
//		newestPose = "(one " + woman + "), (((a blue monster is carrying the sleeping woman)))";
		newestPose = "(((one " + woman + " restrained in air by " + pickRandomString({"red tentacles", "green vines"}) + ")))";
	  
	newestPose += ", ";

	if (newestPose.find("massaging") != std::string::npos)
		pose += "((" + woman + " is lying on a massage table";
	else if (newestPose.find("carry") == std::string::npos && newestPose.find("air") == std::string::npos) {		
		if (outdoors) {
			pose += pickRandomString({
				"((" + woman + " is lying on a lounge chair",
				"((" + woman + " is lying on a hammock",
			});
		} else {
			pose += pickRandomString({
				"((" + woman + " is lying on bed",
				"((" + woman + " is lying on couch",
				"((" + woman + " is lying on a massage table",
				"((" + woman + " is sleeping in a dentist chair))",
				"((" + woman + " is sleeping in a comfy " + pickRandomColorNoDarkGreen() + "chair))",
			});
		}
	}
...
//	}
	...
	mouth.push_back("(((woman is wearing " + pickRandomColorNoBurgundy() + pickRandomString({
...
std::string getShot(std::string output)
{
	...
	if (kNumberOfWomen == 1 && output.find("pov") == std::string::npos) {
		if (output.find("on back") != std::string::npos) {
			...
			newShot.push_back("(((close-up of " + fondleTarget + ":1.5))), (((overhead view:1.3)))");
			...
			if (asleep)
				newShot.push_back("(((view of briefs:1.3))), (sleeping woman is wearing " + pickRandomColor() + pickRandomMaterial() + " panty briefs), ((spread legs))");
			else
				newShot.push_back("(((view of briefs:1.3))), (woman is wearing " + pickRandomColor() + pickRandomMaterial() + " panty briefs), ((spread legs))");
			if (kBodyFocusType == FULL) {
...
//output += getLoras();
	...
	if (output.find("yoga") != std::string::npos)
		std::string toRemove = "((woman's head is resting on a pillow)), ";
		size_t pos = output.find(toRemove);
		if (pos != std::string::npos) {
			output.erase(pos, toRemove.length());
		}
	}
